...
If $f_X$ is the density of a distribution with finite non-zero expectation supported on $\R^{+}$ with respect to some measure $\lambda$, then $f_Y$, the density of the Value-Weighted Transform of this distribution, can be written as:
...
### Pareto distribution
If $X$ has a Pareto distribution with shape parameter $\alpha > 1$ (to ensure a finite expectation) and scale parameter $x_m > 0$, then $f_X(x) \propto x^{-(\alpha + 1)}$ for $x \geq x_m$.

$$
\begin{align*}
\forall x \in [x_m, +\infty), && f_Y(x) &\propto x  \cdot x^{-(\alpha + 1)} \\
&&&\propto x^{-\alpha}
\end{align*}
$$

Which is the density of a Pareto distribution with shape parameter $\alpha - 1$ and scale parameter $x_m$. The VWT preserves the power law tail of the distribution, making it heavier-tailed by reducing the shape parameter by 1. The VWT of a distribution having a heavier tail than the original makes intuitive sense as the transformed distribution is biased towards larger values.

Y has a finite expectation iff. $\alpha > 2$.
...
| Pareto($\alpha$, $x_m$) | Pareto($\alpha - 1$, $x_m$) |
